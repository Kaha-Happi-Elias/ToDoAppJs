/*/const { JSON } = require("express"); //This line is not needed in browser JavaScript
...
const addNote = () => {
    ...
    const uid = new Date().getTime().toString(); // Correct: Call getTime()
...
const toggleComplete = (uid) => {
    ...
    if (index !== -1) {
        ...
        // Add a visual cue (strikethrough)
...
window.addEventListener("load", () => {
    ...
    notesData.forEach((note) => {
        ...
        /*const uid = new Date().getTime().toString();
        const noteObj = {
            completed: "" // Add completed property, initially false
        };
        notesData.push(noteObj);
        localStorage.setItem("notesData", JSON.stringify(notesData));*/
        if (checkbox) checkbox.checked = note.completed; // Set initial state if exists
